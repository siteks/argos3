...
namespace argos
{
   ...
   static const Real CAMERA_ELEVATION = 0.03f;

   static const Real PROXIMITY_SENSOR_RING_ELEVATION = 0.03f;
   ...
   static const Real PROXIMITY_SENSOR_RING_RANGE = 0.04f;
   ...
   static const float sensor_angles[8] =
         {-0.297, -0.855, -1.571, -2.618, 2.618, 1.571, 0.855, 0.297};
   /****************************************/
   /****************************************/
         ...
         m_pcWheeledEntity(NULL)
   {
      try
      {
         ...
         // Epuck sensors are not evenly distributed
         for (int i = 0; i < 8; i++)
         {
            CVector3 pos(PROXIMITY_SENSOR_RING_RADIUS, 0.0f, 0.0f);
            pos.RotateZ(CRadians(sensor_angles[i]));
            CVector3 dir(pos);
            pos += CVector3(0.0f, 0.0f, PROXIMITY_SENSOR_RING_ELEVATION);
            m_pcProximitySensorEquippedEntity->AddSensor(pos,
                                                         dir,
...
//                    m_pcEmbodiedEntity->GetOriginAnchor());
         ...
         // Relative orientation in angle-axis
         CQuaternion cPerspCamOrient(CRadians(0), CVector3::Y);
         SAnchor& cPerspCamAnchor = m_pcEmbodiedEntity->AddAnchor("perspective_camera",
                                                                  CVector3(BODY_RADIUS, 0.0, CAMERA_ELEVATION),
                                                                  cPerspCamOrient);
         cPerspCamAnchor.Enable();
